...
class DeckTestCase(unittest.TestCase):
    def test_deck_generation_clean(self):
        deck = game_engine.generate_deck(None)
        self.assertEqual(len(deck), 35)

        relics = [card for card in deck if card.card_type == "Relic"]
        self.assertEqual(len(relics), 5)

        traps = [card for card in deck if card.card_type == "Trap"]
        self.assertEqual(len(traps), 15)

        treasures = [card for card in deck if card.card_type == "Treasure"]
